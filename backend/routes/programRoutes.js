...
// Public: get all programs for landing page
...
router.post('/', verifyToken, requireAdmin, async (req, res) => {
  try {
    const { name, type, desc, duration, eligibility, start, end, assignTo, link } = req.body;
    if (!name || !type) return res.status(400).json({ message: 'Name and type required.' });
    const ref = await db.collection('programs').add({ name, type, desc: desc || '', duration: duration || '—', eligibility: eligibility || 'All Students', start: start || '', end: end || '', assignTo: assignTo || 'all', link: link || '', createdAt: new Date().toISOString() });
